...
var mapUtils = {
    ...
    geoFacets.forEach(function (geoFacet) {
      ...
      facet.boxes.forEach(function (value, index) {
        if (value.count > 0) {
          ...
          var ptConverted = (0, _proj.fromLonLat)([lng, lat]);
  ...
  convertPropsToGeoJson: function convertPropsToGeoJson(props) {
    ...
    if (props.latitude && props.longitude) {
      var ptConverted = (0, _proj.fromLonLat)([parseFloat(props.longitude), parseFloat(props.latitude)]);
  ...
  convertPointsToGeoJson: function convertPointsToGeoJson(points) {
    ...
    if (points && points.length > 0) {
      points.forEach(function (value, index) {
        ...
        if (latLong.length === 2) {
          var ptConverted = (0, _proj.fromLonLat)([parseFloat(latLong[1]), parseFloat(latLong[0])]);
